...
export class StaticController {
  ...
  serveTestAudio(@Res() res: FastifyReply) {
    const filePath = path.resolve(process.cwd(), 'apps/simultan-dolmetscher/src/assets/test-client.html');
  ...
  serveVoiceRecording(@Res() res: FastifyReply) {
    const filePath = path.resolve(process.cwd(), 'apps/simultan-dolmetscher/src/assets/voice-recording.html');
  ...
  serveLiveAudioEchoClient(@Res() res: FastifyReply) {
    const filePath = path.resolve(process.cwd(), 'apps/simultan-dolmetscher/src/assets/live-audio-echo-client.html');
  ...
  serveAudioWorklet(@Res() res: FastifyReply) {
    const filePath = path.resolve(process.cwd(), 'apps/simultan-dolmetscher/src/assets/audio-worklet.js');
  ...
  serveSocketStream1(@Res() res: FastifyReply) {
    const filePath = path.resolve(process.cwd(), 'apps/simultan-dolmetscher/src/assets/web-socket-stream-test-1.html');
